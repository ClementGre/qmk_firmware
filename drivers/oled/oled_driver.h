...
bool oled_send_cmd(bool screen, const uint8_t *data, uint16_t size);
bool oled_send_cmd_P(bool screen, const uint8_t *data, uint16_t size);
bool oled_send_data(bool screen, const uint8_t *data, uint16_t size);
void oled_driver_init(void);

// Called at the start of oled_init, weak function overridable by the user
// rotation - the value passed into oled_init
// Return new oled_rotation_t if you want to override default rotation
oled_rotation_t oled_init_kb(bool screen, oled_rotation_t rotation);
oled_rotation_t oled_init_user(bool screen, oled_rotation_t rotation);

// Clears the display buffer, resets cursor position to 0, and sets the buffer to dirty for rendering
void oled_clear(bool screen);

// Alias to oled_render_dirty to avoid a change in api.
#define oled_render(screen) oled_render_dirty(screen, false)
...
#else
#    define oled_write_P(screen, data, invert) oled_write(screen, data, invert)
#    define oled_write_ln_P(screen, data, invert) oled_write_ln(screen, data, invert)
#    define oled_write_raw_P(screen, data, size) oled_write_raw(screen, data, size)
...
bool oled_task_user(bool screen);
